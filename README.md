...
- 默认数量：按当前个人空间保存到后端
- 默认并发：按当前个人空间保存到后端
- PiXhost 自动上传开关：按当前个人空间保存到后端

Key 不写入服务器 `data/spaces/{token}/config.json`，`GET /api/config` 也不会返回明文 Key。更换浏览器、清空浏览器数据或换设备登录后，需要在当前浏览器重新填写 Key；历史任务和默认生成设置仍保存在服务器空间里。
...
- 浏览器按当前空间在 `localStorage` 保存 `codex-key` 与 Banana 分组 Key。
...
当前版本使用“空间密码 -> 派生空间 token”的方式打开同一个个人空间：输入同一个空间密码，就会进入同一个空间并看到同一份历史记录。这符合跨设备同步历史的目标，但后续计划升级为更清晰的用户系统：

- 使用“用户名 + 密码”注册和登录。
- 同一个账号在多设备登录后共享任务历史、提示词历史、参考图和输出图。
- 服务器为每个用户绑定内部 `storageToken` 作为数据目录 ID，不再把这个 token 暴露给前端。
- API Key 仍然只保存在当前浏览器本地，不随账号同步。
- 输出图统一通过 `/api/background-tasks/{taskId}/images/{index}` 读取，并校验当前登录用户是否拥有该任务。
- 旧空间可通过“旧空间密码”迁移或绑定到新账号，避免丢失已有历史。

在账号系统落地前，不建议把包含 `/outputs/{spaceToken}/...` 的图片地址分享给他人；后续会把输出图访问收敛到登录态鉴权接口。
